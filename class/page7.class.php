<?php
	...
	class Page7 {
		...
		public function __construct($total, $listRows=25, $query="", $ord=true,$id=''){
			$this->total = $total;
...
// 			@return	object			返回本对象自己$this， 用于连惯操作
		...
		private function firstprev(){
			if($this->page > 1) {
				$str = "&nbsp;<a href='javascript:po({$this->id},1)'>{$this->config["first"]}</a>&nbsp;";
				$str .= "<a href='javascript:po(".($this->page-1).")'>{$this->config['prev']}</a>&nbsp;";		
				return $str;
			}
		...
		private function pageList(){
			...
			for($i = $inum; $i >= 1; $i--){
				...
				if($page >= 1)
					$linkPage .= "<a href='javascript:po({$page})'>{$page}</a>&nbsp;";
			}
			/*当前页的信息 */
			if($this->pageNum > 1)
		...
		private function nextlast(){
			if($this->page != $this->pageNum) {
				$str = "&nbsp;<a href='javascript:po(".($this->page+1).")'>{$this->config['next']}</a>&nbsp;";
				$str .= "&nbsp;<a href='javascript:po({".($this->pageNum).")'>{$this->config['last']}</a>&nbsp;";
				return $str;
			}
		}
