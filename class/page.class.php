	...
	*/
	class Page {
		...
		private function firstprev(){
			if($this->page > 1) {
				$str = "&nbsp;<a href='javascript:pagejs(1)'>{$this->config["first"]}</a>&nbsp;";
				$str .= "<a href='javascript:pagejs(".($this->page-1).")'>{$this->config['prev']}</a>&nbsp;";		
				return $str;
			}
		...
		private function pageList(){
			...
			for($i = $inum; $i >= 1; $i--){
				...
				if($page >= 1)
					$linkPage .= "<a href='javascript:pagejs({$page})'>{$page}</a>&nbsp;";
			}
			/*当前页的信息 */
			if($this->pageNum > 1)
		...
		private function nextlast(){
			if($this->page != $this->pageNum) {
				$str = "&nbsp;<a href='javascript:pagejs(".($this->page+1).")'>{$this->config['next']}</a>&nbsp;";
				$str .= "&nbsp;<a href='javascript:pagejs(".($this->pageNum).")'>{$this->config['last']}</a>&nbsp;";
				return $str;
			}
		}
