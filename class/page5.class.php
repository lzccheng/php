<?php
	...
	*/
	class Page5 {
		 ...
		 */
		public function __construct($total, $listRows=25, $query="", $ord=true,$id=1){
			$this->total = $total;
		...
		private function firstprev(){
			if($this->page > 1) {
				$str = "&nbsp;<a href='javascript:pagecha({$this->id},1)'>{$this->config["first"]}</a>&nbsp;";
				$str .= "<a href='javascript:pagecha({$this->id},".($this->page-1).")'>{$this->config['prev']}</a>&nbsp;";		
				return $str;
			}
		...
		private function pageList(){
			...
			for($i = $inum; $i >= 1; $i--){
				...
				if($page >= 1)
					$linkPage .= "<a href='javascript:pagecha({$this->id},{$page})'>{$page}</a>&nbsp;";
			}
			/*当前页的信息 */
			if($this->pageNum > 1)
		...
		private function nextlast(){
			if($this->page != $this->pageNum) {
				$str = "&nbsp;<a href='javascript:pagecha({$this->id},".($this->page+1).")'>{$this->config['next']}</a>&nbsp;";
				$str .= "&nbsp;<a href='javascript:pagecha({$this->id},".($this->pageNum).")'>{$this->config['last']}</a>&nbsp;";
				return $str;
			}
		}
